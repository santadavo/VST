...
Require Import compcert.lib.Integers. (*for byte*)
...
Definition data:Type:= byte.
Lemma data_dec: forall (d1 d2: data), { d1=d2 } + { ~  d1=d2 }. Proof. apply Byte.eq_dec. Qed. 
Lemma listdata_dec: forall (d1 d2: list data), { d1=d2 } + { ~  d1=d2 }. 
Proof. apply (list_eq_dec data_dec). Qed.

(*Constants defined in include/sha.h, md4.h, md5.h *)
...
Definition MD4_CBLOCK:Z := 64.
Definition MD4_DIGEST_LENGTH:Z := 16.

Definition MD5_CBLOCK:Z := 64.
Definition MD5_DIGEST_LENGTH:Z := 16.

Definition SHA1_CBLOCK:Z := 64.
Definition SHA1_DIGEST_LENGTH:Z := 20.

Definition SHA224_CBLOCK:Z := 64.
Definition SHA224_DIGEST_LENGTH:Z := 28.

Definition SHA384_CBLOCK:Z := 128.
Definition SHA384_DIGEST_LENGTH:Z := 48.

Definition SHA512_CBLOCK:Z := 128.
Definition SHA512_DIGEST_LENGTH:Z := 64.
...
Definition EVP_get_digestbynid (nid:DigestNID): option EVP_MD :=
  match nid with
    NID_md4 => Some md4_md
  | NID_md5 => Some md5_md
  | NID_sha1 => Some sha1_md
  | NID_sha224 => Some sha224_md
  | NID_sha256 => Some sha256_md
  | NID_sha384 => Some sha384_md
  | NID_sha512 => Some sha512_md
  | NID_md5_sha1 => Some md5_sha1_md
  | _ => None
  end.
...
Definition mddataTp R:Type:= match R with md5_sha1_md => list data * list data | _ => list data end.

Definition md_ctxt_size (R: EVP_MD): Z :=
  match R with 
  md4_md => 92
| md5_md => 92
| sha1_md => 96
...
Definition UpdateTP R := mddataTp R -> list data -> mddataTp R.
Definition FinalTP R:=mddataTp R -> list data (*result*).

(*See digest/internal.h*)
Record EVP_MD_record R := 
{ EVP_MD_rec_type: DigestNID;
  EVP_MD_rec_md_size:Z;
  EVP_MD_rec_flags:FlagsTP;
  EVP_MD_rec_init: InitTP R; 
  EVP_MD_rec_update: UpdateTP R;
  EVP_MD_rec_final: FinalTP R;
...
Definition SHA256_MD: EVP_MD_record sha256_md.
  econstructor.
  apply NID_sha256.
...
Parameter MD4: list data -> list data.
Definition MD4_MD: EVP_MD_record md4_md.
  econstructor.
  apply NID_md4.
  apply MD4_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply MD4. }
  apply MD4_CBLOCK.
  apply (md_ctxt_size md4_md).
Defined.

Parameter MD5: list data -> list data.
Definition MD5_MD: EVP_MD_record md5_md.
  econstructor.
  apply NID_md5.
  apply MD5_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply MD5. }
  apply MD5_CBLOCK.
  apply (md_ctxt_size md5_md).
Defined.

Parameter SHA1: list data -> list data.
Definition SHA1_MD: EVP_MD_record sha1_md.
  econstructor.
  apply NID_sha1.
  apply SHA1_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply SHA1. }
  apply SHA1_CBLOCK.
  apply (md_ctxt_size sha1_md).
Defined.

Parameter SHA224: list data -> list data.
Definition SHA224_MD: EVP_MD_record sha224_md.
  econstructor.
  apply NID_sha224.
  apply SHA224_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply SHA224. }
  apply SHA224_CBLOCK.
  apply (md_ctxt_size sha224_md).
Defined.

Parameter SHA384: list data -> list data.
Definition SHA384_MD: EVP_MD_record sha384_md.
  econstructor.
  apply NID_sha384.
  apply SHA384_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply SHA384. }
  apply SHA384_CBLOCK.
  apply (md_ctxt_size sha384_md).
Defined.

Parameter SHA512: list data -> list data.
Definition SHA512_MD: EVP_MD_record sha512_md.
  econstructor.
  apply NID_sha512.
  apply SHA512_DIGEST_LENGTH.
  apply 0.
  (*Init*) { apply nil. }
  (*Update*) { red. apply app. }
  (*Final*) { red. apply SHA512. }
  apply SHA512_CBLOCK.
  apply (md_ctxt_size sha512_md).
Defined.

Definition MD5_SHA1_MD: EVP_MD_record md5_sha1_md.
  econstructor.
  apply NID_md5_sha1.
  apply (MD5_DIGEST_LENGTH + SHA1_DIGEST_LENGTH).
  apply 0.
  (*Init*) { apply (nil, nil). }
  (*Update*) { red. simpl. intros [A B] l. apply (app A l, app B l). }
  (*Final*) { red. simpl. intros [A B]. apply (app A B). }
  apply 64.
  apply (md_ctxt_size md5_sha1_md).
Defined.

Definition EVPMD_record_of_EVPMD (md:EVP_MD): EVP_MD_record md :=
  match md with 
  md4_md => MD4_MD
| md5_md => MD5_MD
| sha1_md => SHA1_MD
...
(*TODO: complete*)
Definition get_md_record (md:EVP_MD): option (EVP_MD_record md):=
  match md with
    md4_md => Some MD4_MD
...
Lemma get_md_record_sound md r (R:get_md_record md = Some r):
      EVP_get_digestbynid (EVP_MD_rec_type md r) = Some md.
Proof. destruct md; inv R; trivial. Qed.


Definition type_of_MD (D:EVP_MD): DigestNID := EVP_MD_rec_type _ (EVPMD_record_of_EVPMD D).
Definition md_size_of_MD (D:EVP_MD): Z := EVP_MD_rec_md_size _ (EVPMD_record_of_EVPMD D).
Definition flags_of_MD (D:EVP_MD): FlagsTP := EVP_MD_rec_flags _ (EVPMD_record_of_EVPMD D).
Definition block_size_of_MD (D:EVP_MD): Z := EVP_MD_rec_block_size _ (EVPMD_record_of_EVPMD D).
Definition ctx_size_of_MD (D:EVP_MD): Z := EVP_MD_rec_ctx_size _ (EVPMD_record_of_EVPMD D).
...
Definition EVP_MD_type md r := bind (get_md_record md r) (EVP_MD_rec_type md).
